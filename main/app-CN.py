...
app.layout = html.Div([
    ...
    html.H1('交互式快速气候信号检测软件 IRCSD', style={'textAlign': 'center', 'fontSize': '30px', 'marginTop': '0'}),
    html.P([
        html.Strong('Author: '),
    ...
    html.B("1. 上传待检测气候信号", style={'color':"#de420d"}),
    html.Br(),
    html.Label('输入信号时间分辨率:  '),  # RI时间分辨率声明 
    fac.AntdRadioGroup(
        id='RI_frequeny_dropdown',
        options=[
            {'label': '年平均', 'value': 'year'},
            {'label': '月平均', 'value': 'month'}
        ],
        value='year'
        ),
    html.Br(),
    ...
    du.Upload(
        id='uploader',
        text='点击或拖动文件至此处进行上传！（仅支持.nc和.txt格式文件。输入信号频率必须为年或月）',
    ...
    html.B("2. 选择时间范围", style={'color':"#de420d"}),
    html.Br(),
    dcc.Checklist(id='RI_time_sel',
                  options=[{'label': '使用原始时间范围', 'value': 'orgin'}],
                  value=['orgin']),  # 默认原时间范围
    # 时间范围选择
    html.Div(id='time-range-container', style={'display': 'none'}, children=[
        fac.AntdRow(
            [
                fac.AntdCol(
                    html.Div([
                        html.Label('选择开始年份:', style={'fontSize': '16px'}),
                ...
                fac.AntdCol(
                    html.Div([
                        html.Label('选择结束年份:', style={'fontSize': '16px'}),
    ...
    html.B("3. 选择分析时间", style={'color':"#de420d"}), # 数据预处理方式选择
    html.Br(),
    fac.AntdRow([fac.AntdCol(
        html.Div([
                html.Label('待检测信号:'),  # RI时间尺度选择
                dcc.Dropdown(
    ...
    html.B("4. 选择数据预处理方法", style={'color':"#de420d"}), # 数据预处理方式选择
    ...
    html.B("5. 去趋势", style={'color':"#de420d"}),# 去趋势选择

    fac.AntdRow([
    dcc.Checklist(id='RI_detrend_checklist',
                  options=[{'label': '去除待检测信号趋势', 'value': 'detrend'}],
                  value=[]),  # 默认不去趋势
    dcc.Checklist(id='CI_detrend_checklist',
                  options=[{'label': '去除气候指数趋势', 'value': 'detrend'}],
                  value=[])
    ]),  # 默认不去趋势

    html.B("6. 滤波", style={'color':"#de420d"}),# 滤波选择
    html.Br(),
    html.Label('是否同时滤波待检测信号和气候指数: '),
    fac.AntdRadioGroup(id='same_filter_method',
                 options=[
                     {'label': '同时使用相同滤波方法 (包括不滤波)', 'value': 'Samefilter'},
                     {'label': '待检测信号不滤波', 'value': 'Notsamefilter'}
                 ],
                 value='Samefilter'),
    html.Br(),
    ...
    html.Label('选择滤波方法 (暂不支持Lanczos滤波，推荐使用不滤波或Butterworth滤波):  '),
    ...
    fac.AntdRadioGroup(id='filter_type_dropdown',
                 options=[
                     {'label': '低通滤波', 'value': 'low'},
                     {'label': '高通滤波', 'value': 'high'},
                     {'label': '带通滤波', 'value': 'band'}
                 ],
                 value='low'),
    html.Br(),             
    html.Label('默认待检测信号频率=1, 截止频率1='),
    ...
    html.B("7. 选择计算的相关系数", style={'color':"#de420d"}), 
    ...
    html.B("8. 选择检测模式", style={'color':"#de420d"}), 
    html.Div([
    dcc.RadioItems(
        id='output-type',
        options=[
            {'label': '检测输入信号与所有气候指数', 'value': 'batch'},
            {'label': '检测输入信号与单个气候指数', 'value': 'single'}
        ],
    ...
    html.Button('开始检测输入信号', id='submit_button', n_clicks=0, style={'font-size': '15px', 'width': '200px', 'display': 'inline-block', 'margin-bottom': '10px', 'margin-top': '10px','margin-right': '5px', 'height':'35px', 'verticalAlign': 'top'}),
    ...
    if stored_data is None:
        ...
        if output_type == "single":
            ...
            if correlation_result == None:
                return "", f"状态: 气候指数{index_sel}的时间范围与输入信号不匹配!"
            else:
                fig = go.Figure()
                if same_filter == "Samefilter": 
                    if fil_method == 'movingaverage':
                ...
                return dcc.Graph(figure=fig), f"状态: 检测和分析输入信号与{index_sel}完成!"
        ...
        ### 批量绘图 ###
            ...
            return dcc.Graph(figure=fig), "状态: 检测和分析输入信号完成!"
