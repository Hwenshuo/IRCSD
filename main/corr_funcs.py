...
def lagged_correlation(da1, da2, max_lag):
    lags = np.arange(-max_lag, max_lag + 1)
    correlations = []
    # 循环计算每个滞后下的相关系数
    for lag in lags:
        if lag < 0: # da1 超前，da2 滞后
            r, _ = pearsonr(da1[:lag].values, da2[-lag:].values)
        elif lag > 0: # da1 滞后，da2 超前
            
            r, _ = pearsonr(da1[lag:].values, da2[:-lag].values)
        else: # 无滞后，计算原始序列的相关性
            r, _ = pearsonr(da1.values, da2.values)
...
def sliding_correlation(data1, data2, window_size, dim="time"):
    """
    计算两个 xarray.DataArray 对象的滑动相关系数。

    参数:
    - data1 (xarray.DataArray): 第一个数据数组。
    - data2 (xarray.DataArray): 第二个数据数组。
    - window_size (int): 滑动窗口大小。
    - dim (str): 要应用滑动窗口的维度名称。

    返回:
    - xarray.DataArray: 包含滑动相关系数的数组。
    """
    # 构建滑动窗口
    data1_rolling = data1.rolling({dim: window_size}, center=True).construct("window_dim")
    data2_rolling = data2.rolling({dim: window_size}, center=True).construct("window_dim")

    # 计算滑动相关系数
    sliding_corr = xs.pearson_r(data1_rolling, data2_rolling, dim="window_dim")
    return sliding_corr
...
def ESS_DS1976(x, y, max_lag=None):
    ...
    if max_lag is None:
        max_lag = n // 2
    T = 0
    # 使用 autocorr 函数计算自相关，并提取滞后值
    Rxx = autocorr(x, dim="time",nlags=max_lag+1)
    Ryy = autocorr(y, dim="time",nlags=max_lag+1)
...
#暂时废弃
def ESS_to_r_value(Neff, alpha=0.05):
    # 计算自由度
    dof = Neff - 2
    # 计算临界 t 值（双尾）
    t_critical = stats.t.ppf(1 - alpha / 2, df=dof)
    # 计算临界 r 值
    r_critical = t_critical / np.sqrt(t_critical**2 + dof)
    return r_critical

def sig_to_r_value(N, alpha=0.05):
    # 计算自由度
    dof = N - 2
    # 计算临界 t 值（双尾）
    t_critical = stats.t.ppf(1 - alpha / 2, df=dof)
